sub init()
    ...
    m.buttongroup.buttons = ["Start here","Open Dialog Box","Open On-Screen Keyboard","Finish Here"]
...
function onKeyEvent(key as String, press as Boolean) as Boolean
    handled = false
    'print "item in focus is " m.top.focusedChild
    if (press) then
        if (key="ok") then
            print "ok key hit"
            handled = true
...
sub displayKeyboard()
    ...
    m.keyboardDialog.title="This is an on-screen keyboard"
    m.keyboardDialog.optionsDialog=true
    print "m.keyboardDialog" m.keyboardDialog
    print "m.keyboardDialog.keyboard" m.keyboardDialog.keyboard
    m.keyboardDialog.observeField("text","onTextChange")
    m.keyboardDialog.buttons=["OK","Cancel"]
    m.keyboardDialog.observeField("buttonSelected","onKeyboardConfirmSelection")
    m.top.dialog=m.keyboardDialog
    m.keyboardDialog.setFocus(true) 
end sub
sub onKeyboardConfirmSelection()
    print "a keyboard button was pressed"
    buttonFocused = m.top.dialog.buttonFocused 
    print "m.keyboardDialog.buttons[buttonFocused] "  m.keyboardDialog.buttons[buttonFocused]
    if (m.keyboardDialog.buttons[buttonFocused]="OK")
        print "keyboard OK was pressed"
        m.label.text="You entered text " + m.keyboardDialog.text
    else if (m.keyboardDialog.buttons[buttonFocused]="Cancel")
        print "keyboard Cancel was pressed"
        m.label.text="keyboard entry was cancelled"
    end if 
...
sub onTextChange()
    print "text is being entered " m.keyboardDialog.text
end sub