...
Sub BTS__StorePerformanceData(name as String, value as Object)
    ...
    print "PERF_DATA: " + m.testInstance.Name + ": " + timestamp + ": " + name + "|" + TF_Utils__AsString(value)
End Sub
...
sub Logger__SetServerURL(url = "" as String)
    print "This function is deprecated. Please use Logger__SetServer(host, port)"
end sub
...
sub Logger__SendToServer(statObj as Object)
    if TF_Utils__IsNotEmptyString(m.serverURL) and TF_Utils__IsValid(statObj)
        print "***"
        print "***   Sending statsObj to server: "; m.serverURL
        
        request = CreateObject("roUrlTransfer")
        request.SetUrl(m.serverURL)
        statString = FormatJson(statObj)
        
        print "***   Response: "; request.postFromString(statString)
        print "***"
        print "******************************************************************"
    end if
end sub
...
sub Logger__PrintStatistic(statObj as Object)
            ...
            for each testSuite in statObj.Suites
                for each testCase in testSuite.Tests
                    if m.verbosity = m.verbosityLevel.verboseFailed and testCase.result = "Fail"
                        ...
                        print "***   "; testSuite.Name; ": "; testCase.Name; " - "; testCase.Result
                    end if
                end for
            end for
    ...
    print "***"
    print "***   Total  = "; TF_Utils__AsString(statObj.Total); " ; Passed  = "; statObj.Correct; " ; Failed   = "; statObj.Fail; " ; Skipped   = "; statObj.skipped; " ; Crashes  = "; statObj.Crash;
    ...
    if TF_Utils__IsAssociativeArray(statSuiteObj) and TF_Utils__IsAssociativeArray(statTestObj)
        ...
        if m.echoEnabled
            if m.verbosity = m.verbosityLevel.normal
                print "***   "; statSuiteObj.Name; ": "; statTestObj.Name; " - "; statTestObj.Result
            else if m.verbosity = m.verbosityLevel.verbose
...
sub Logger__PrintSuiteStatistic(statSuiteObj as Object)
    ...
    print "==="
    print "===   Total  = "; TF_Utils__AsString(statSuiteObj.Total); " ; Passed  = "; statSuiteObj.Correct; " ; Failed   = "; statSuiteObj.Fail; " ; Skipped   = "; statSuiteObj.skipped; " ; Crashes  = "; statSuiteObj.Crash;
    print " Time spent: "; statSuiteObj.Time; "ms"
    print "==="
...
sub Logger__PrintTestStatistic(statTestObj as Object)
    ...
    print "---   Result:        "; statTestObj.Result
    print "---   Time:          "; statTestObj.Time

    if LCase(statTestObj.result) = "skipped"
        if Len(statTestObj.message) > 0
            print "---   Message: "; statTestObj.message
        end if
    else if LCase(statTestObj.Result) <> "success"
        print "---   Error Code:    "; statTestObj.Error.Code
        print "---   Error Message: "; statTestObj.Error.Message
    end if
        ...
        for each testNodeName in testNodes
            ...
            if testNode <> invalid
                ...
                if CreateObject("roSGScreen").CreateScene(testNodeName) <> invalid
                    print "WARNING: Test cases cannot be run in main scene."
    ...
    processors = {
        ...
        repeatedtest: sub()
            ...
            if allowedAnnotationsRegex.IsMatch(annotationLine)
                ...
                print "WARNING: Wrong format of repeatedTest(numberOfRuns) "annotationLine
            end if
        end sub
                ...
                if m.annotations.methodSource <> invalid and m.annotations.parameterizedTest <> invalid then
                    ...
                    if allowedAnnotationsRegex.IsMatch(methodAnottation)
                        ...
                        if providerFunctionPointer <> invalid then
                            ...
                            print "WARNING: Cannot find function [" providerFunction "]"
                        end if
                    end if
                else
                    print "WARNING: Wrong format of  @ParameterizedTest \n @MethodSource(providerFunctionName)"
                    print "m.executedParametrizedAdding = "m.executedParametrizedAdding
                    print "m.annotations.methodSource = "m.annotations.methodSource
                    print "m.annotations.parameterizedTest = "m.annotations.parameterizedTest
                    print ""
                end if
            end if
        end sub
            ...
            if allowedAnnotationsRegex.IsMatch(line)
                ...
                if currentAnottations.count() > 0 then
                    ...
                    if voidFunctionRegex.IsMatch(line) or isParametrized then
                        ...
                        print "WARNING: annotation " currentAnottations " isparametrized=" isParametrized " skipped at line " index ":[" line "]"
                        processors.annotations = {}
                        currentAnottations = []
                    end if
                end if
...
sub TestRunner__SetIncludeFilter(listOfFunctions as Dynamic)
    ...
    if TF_Utils__IsArray(listOfFunctions)
        ...
        print "WARNING: Could not parse input parameters for Include Filter. Filter wont be applied."
    end if
end sub
...
sub TestRunner__SetExcludeFilter(listOfFunctions as Dynamic)
    ...
    if TF_Utils__IsArray(listOfFunctions)
        ...
        print "WARNING: Could not parse input parameters for Exclude Filter. Filter wont be applied."
    end if
end sub
...
function TestRunner__GetTestFilesList(testsDirectory = m.testsDirectory as String, testFilePrefix = m.testFilePrefix as String) as Object
    ...
    if testsDirectory <> ""
        ...
        if m.isNodeMode
            print String(2, Chr(10))
            print String(10, "!!!")
            print "Note if you crash here this means that we are in render thread and searching for tests"
            print "Problem is that file naming is wrong"
            print "check brs file name they should match pattern ""Test_ExactComponentName_anything.brs"""
            print "In this case we were looking for "testFilePrefix
            print String(10, "!!!") String(2, Chr(10))
        end if
